...
func TestSearchStories(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			srv := NewServer(logr.Discard(), domainmock.NewMockSettingsRepository(ctrl), domainmock.NewMockVocabRepository(ctrl), nil, nil, nil, es, nil, nil, nil, nil, nil, "")
...
func TestGetStoryTokens(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			srv := NewServer(logr.Discard(), domainmock.NewMockSettingsRepository(ctrl), vr, sr, nil, nil, es, nil, nil, nil, nil, nil, "")
			req := httptest.NewRequest(http.MethodGet, tt.url, nil)
			w := httptest.NewRecorder()
